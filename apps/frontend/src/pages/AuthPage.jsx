...
export function AuthPage() {
  ...
  function otpNotice(response) {
    if (response.devOtps) {
      return `Demo email OTP: ${response.devOtps.emailOtp} | Demo mobile OTP: ${response.devOtps.mobileOtp}`;
    }
  ...
  async function resendOtp() {
    ...
    setLoading(true);
    try {
      const response = await resendSignupOtp({ identifier, channel: "both" });
      setPendingOtp(response);
      setNotice(otpNotice(response));
    } catch (err) {
    ...
    <main className="relative min-h-dvh bg-slate-50 px-4 py-4 pt-20 dark:bg-slate-950 md:grid md:place-items-center md:pt-4">
      ...
      <section className="surface mx-auto grid w-full max-w-sm grid-cols-1 overflow-hidden rounded-lg shadow-panel md:max-w-5xl md:grid-cols-[1fr_0.86fr]">
        <div className="p-5 md:p-12">
          ...
          <h1 className="mb-3 text-3xl font-black leading-tight md:text-4xl">AI finance command center</h1>
          <p className="max-w-xl text-sm leading-6 text-slate-600 dark:text-slate-300 md:text-base">Track spending, scan bills, decode UPI messages, and turn messy transactions into practical financial decisions.</p>
          ...
          <div className="mt-5 grid gap-2 text-sm text-slate-600 dark:text-slate-300 md:mt-8 md:gap-3">
            ...
            <span className="flex items-center gap-2"><ShieldCheck size={18} /> OTP verification for new accounts</span>
          </div>
        </div>
          ...
          <div className="mb-5 flex rounded-md bg-slate-100 p-1 dark:bg-slate-900">
            {[
              ...
              <button
                ...
                className={`flex-1 rounded-md px-3 py-2 text-sm font-bold ${mode === key ? "bg-white shadow dark:bg-slate-800" : "text-slate-500"}`}
              >
                {label}
              </button>
            ))}
          ...
          {pendingOtp ? (
            <form onSubmit={verifyOtp} className="grid gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500 dark:text-slate-300">Enter the OTP sent to your email and mobile number.</p>
              </div>
              <Input
                label="Email OTP"
                inputMode="numeric"
                ...
                onChange={(e) => setOtpForm((current) => ({ ...current, emailOtp: e.target.value }))}
              />
              <Input
                label="Mobile OTP"
                inputMode="numeric"
                ...
                onChange={(e) => setOtpForm((current) => ({ ...current, mobileOtp: e.target.value }))}
              />
            ...
            <form onSubmit={submit} className="grid gap-4">
              ...
              {notice && <p className="rounded-md bg-blue-50 px-3 py-2 text-sm font-semibold text-blue-700 dark:bg-blue-950 dark:text-blue-200">{notice}</p>}
              {error && <p className="text-sm text-coral">{error}</p>}
              <Button type="submit" disabled={loading || !form.password || !identifier}>
              ...
              <Button type="button" variant="secondary" onClick={continueWithGoogle} disabled={loading}>
